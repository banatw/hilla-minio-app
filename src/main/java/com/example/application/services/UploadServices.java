...
public class UploadServices {
    ...
    public byte[] getFile(String filename) throws InvalidKeyException, ErrorResponseException, InsufficientDataException, InternalException, InvalidResponseException, NoSuchAlgorithmException, ServerException, XmlParserException, IOException {
     ...
     @NonNull
    public String getFileBase64(String filename) throws InvalidKeyException, ErrorResponseException, InsufficientDataException, InternalException, InvalidResponseException, NoSuchAlgorithmException, ServerException, XmlParserException, IOException {
        GetObjectArgs args = GetObjectArgs.builder()
                        .bucket("my-bucket")
                        .object(filename)
                        .build();
            try(InputStream rStream = minioClient.getObject(args)) {
                return Base64.getEncoder().encodeToString(IOUtils.toByteArray(rStream));
            }
    }

    public void uploadDefaultFile(String fileName,String mimeType) {
        try (InputStream inputStream = getClass().getResourceAsStream("/META-INF/resources/images/default.png")) {
                MockMultipartFile mockMultipartFile = new MockMultipartFile(fileName,fileName,mimeType,inputStream);
